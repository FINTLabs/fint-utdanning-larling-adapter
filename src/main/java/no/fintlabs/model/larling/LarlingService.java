...
public class LarlingService {
  ...
  public List<LarlingResource> getLarlingResources() {
    try {
      List<LarlingResource> larlingResources =
  ...
  private LarlingResource createLarlingResource(Contract contract) {
    ...
    if (!contract.getType().isEmpty()) larlingResource.setKontraktstype(contract.getType());
    ...
    larlingResource.addPerson(
        Link.with(PersonResource.class, "fodselsnummer", contract.getElev().getFodselsNummer()));
    larlingResource.addBedrift(
        Link.with(VirksomhetResource.class, "virksomhetsid", contract.getBedriftsNummer()));
    larlingResource.addProgramomrade(
        Link.with(
            ProgramomradeResource.class,
            "utdanning/utdanningsprogram/programomrade/systemid",
            setProgramKode(contract.getProgramKode())));
    larlingResource.addSelf(
