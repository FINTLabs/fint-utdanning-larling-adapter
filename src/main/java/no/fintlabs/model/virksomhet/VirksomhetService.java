...
public class VirksomhetService {
  ...
  public List<VirksomhetResource> getVirksomhetResources() {
    try {
      ...
      List<VirksomhetResource> virksomhetResources =
  ...
  private void addLarlingLinks(VirksomhetResource virksomhetResource, String bedriftsNummer) {
    cacheService
        .getContracts(bedriftsNummer)
        .forEach(
            contract ->
                virksomhetResource.addLarling(
                    Link.with(
                        LarlingResource.class, "systemid", contract.getElev().getSystemId())));
  }
